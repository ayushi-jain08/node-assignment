...
export const CreatePost = async (req, res, next) => {
  ...
  try {
    ...
    logger.error(`Error at CreatePost controller ${error}`);
    next(error);
  }
};
...
export const UpdatePost = async (req, res, next) => {
  ...
  try {
    ...
    logger.error(`Error at UpdatePost controller ${error}`);
    next(error);
  }
};
  ...
  try {
    ...
    if (is_post_exist.owner.toString() !== req.user.id) {
      return next(
        errorHandler(
          httpCode.forbidden_code,
          "You have not permission to delete this post"
        )
      );
    }
    ...
    logger.error(`Error at DeletePost controller ${error}`);
    next(error);
  }
};
...
export const getAllPost = async (req, res) => {
  try {
    ...
    logger.error(`Error at getAllPost controller ${error}`);
...
export const getPost = async (req, res) => {
  try {
    ...
    logger.error(`Error at getPost controller ${error}`);
    return Res(res, httpCode.forbidden_code, "Internal Server Error");
  }
};
